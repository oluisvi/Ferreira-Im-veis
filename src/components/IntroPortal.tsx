...
function shouldPlayIntro() {
  ...
  try {
    ...
    // If storage is unavailable, keep the intro non-blocking and continue normally.
  }
...
function markIntroAsSeen() {
  ...
  try {
    ...
    // Storage can fail in privacy-restricted environments; the site must still unlock.
  }
}
...
export function IntroPortal({ waitForReady = waitForCriticalContent }: IntroPortalProps = {}) {
  ...
  useEffect(() => {
    readinessRef.current = waitForReady
  }, [waitForReady])
  ...
  useEffect(() => {
    ...
    const criticalContentReady = Promise.resolve()
      .then(() => readinessRef.current())
      .catch(() => undefined)

    void Promise.race([
      Promise.all([minimumHold, criticalContentReady]),
      safeTimeout,
    ]).then(async () => {
      if (cancelled) return
  ...
  return (
    <div
      className={`intro-portal intro-portal--${phase}`}
      data-state={phase}
      data-testid="intro-portal"
      aria-hidden="true"
    >
