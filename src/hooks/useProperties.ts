...
import { loadProperties, type CatalogSource, type Property } from '../data/propertyCatalog'

type PropertyState = { properties: Property[]; source: CatalogSource; loading: boolean }

export function useProperties() {
  const [state, setState] = useState<PropertyState>({ properties: [], source: 'fallback', loading: true })
  useEffect(() => {
    let cancelled = false
