...
const parseInteger = (value) => { const parsed = parseLocaleNumber(value); return parsed === null ? null : Math.round(parsed) }
const parseBoolean = (value) => new Set(['sim', 's', 'yes', 'true', '1', 'destaque']).has(normalizeKey(value))
const splitList = (value) => !value ? [] : String(value).split(/\s*\|\s*|\r?\n/).map((item) => item.trim()).filter(Boolean)
...
function createRecord(headers, row) {
  return headers.reduce((record, header, index) => { record[normalizeKey(header)] = String(row[index] ?? '').trim(); return record }, {})
}
